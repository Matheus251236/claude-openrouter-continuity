...
writeFileSync(join(root, 'runtime/report.html'), `<!doctype html><html lang="pt-BR"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Claude Continuity — verificação</title><style>body{font:17px/1.6 system-ui;max-width:960px;margin:50px auto;padding:0 24px;background:#f5f2eb;color:#242924}h1{line-height:1.15}aside{border-left:5px solid #b46b22;padding:16px;background:#fff4df}pre{font:13px/1.7 monospace;background:#fff;padding:22px;overflow:auto;border-radius:12px}.status{color:${pass ? '#216c4a' : '#aa2828'};font-weight:700}</style><p>PROTÓTIPO EXPERIMENTAL · 0.1.0</p><h1>Claude OpenRouter Continuity</h1><p class="status">${pass ? 'Testes locais aprovados' : 'Verificação encontrou falhas'}</p><aside><strong>A sessão real do Claude Desktop ainda não está conectada.</strong> Estes testes usam respostas e credenciais fictícias. Não comprovam a troca em sua conta e não consomem tokens.</aside><p>O transporte testa a preservação das mensagens e referências de ferramentas ao trocar após um erro de limite. O plugin registra o erro e informa o estado da integração. A conexão ao processo do Desktop é a etapa pendente.</p><p>Executado em ${escape(new Date().toISOString())}.</p><pre>${escape(logs)}</pre></html>`, 'utf8');
